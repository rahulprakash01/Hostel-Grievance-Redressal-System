...
exports.userRegister = asyncWrapper( async (req, res) => {
    ...
    try {
      ...
      //   block_id=4567;
      // }
      // console.log("Back"+block_Id);
      const block_name="A";
      await db.pool.query(
        "INSERT INTO block (block_id, block_name) VALUES ($1, $2) RETURNING *",
        [block_id, block_name]
      );
      ...
      if (type === "student") {
        ...
        console.log("backend me block id usn and room hai jee ->"+block_id+" "+usn +" "+ room);
        console.log(newUser.rows);
        await db.pool.query(
          "INSERT INTO student (student_id, block_id, usn, room) VALUES ($1, $2, $3, $4) RETURNING *",
