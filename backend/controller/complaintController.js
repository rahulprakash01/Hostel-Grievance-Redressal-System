...
const decodeUser = async (token) => {
  try {
    ...
    if (type === "student") {
      ...
      console.log("res ka row",result.rows);
      return result.rows[0];
      if (result.rows.length > 0) {
        userInfo = result.rows[0];
        consolg.log("user info hai je -> ",userInfo);
      }
      else {
        console.log("No student found with user_id:", user_id);
      }
    }
    ...
    if (type === "warden") {
      ...
      if (result.rows.length > 0) {
        userInfo = result.rows[0];
        // consolg.log("user info hai jee -> ",userInfo);
      }
...
exports.postComplaints = asyncWrapper(async (req , res)=> {
    try {
      ...
      console.log("complain ke body ->",req.body);
        ...
        console.log("error aaya hai:",err.message);
      }
}); 
