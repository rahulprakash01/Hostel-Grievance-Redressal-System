...
function Register() {
  ...
  const [usn, setUsn] = useState("1234");
  const [room, setRoom] = useState("456");
  const onSubmit = async(e) => {
    ...
    try {
      ...
      if (role === Roles.WARDEN) {
        ...
        const response = await fetch("https://hostel-grievance-redressal-system.onrender.com/register", {
            method: "POST",
            headers: {"content-type": "application/json "},
            body : JSON.stringify(body),
        });
        console.log("usn hai jee->",usn+ "  "+room);
        console.log(body)
